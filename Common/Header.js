...
useEffect(()=>{
  ...
  return (
    ...
    <button className='px-5 bg-primary/10 text-primary border-primary my-4 text-xs hover:bg-primary/30' style={{border:"solid 0.5px",fontWeight:"400"}} onClick={()=>
      ...
      <div className={`${showNav?"h-screen text-[12px]":"h-[60px]"} transition-all ease-in-out duration-500 z-20   md:hidden bg-background-header/25 w-full fixed top-0 font-montserrat backdrop-blur-3xl px-8 md:px-16 flex flex-col`}>
        ...
        {cRoutes.map((obj, i) => (
          <div className={`uppercase cursor-pointer text-[20px] text-green-400 font-normal flex justify-center items-center ${obj.path.includes(router) ? "border-b-2 border-primary " : ""} `}
           onClick={()=>{
            if(i===3&&!token){
              route.push(`/login`);
            } 
